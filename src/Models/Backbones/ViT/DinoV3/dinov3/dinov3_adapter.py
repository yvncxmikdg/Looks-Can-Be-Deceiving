...
class Extractor(nn.Module):
    ...
    def forward(self, query, reference_points, feat, spatial_shapes, level_start_index, H, W):
        ...
        if self.with_cp and query.requires_grad:
            # use_reentrant=False is required under DDP: the reentrant implementation (torch's
            # default in 2.x) re-runs this block inside the backward pass, which fires the DDP
            # reducer's autograd hooks a second time and aborts the step with "Expected to mark a
            # variable ready only once". The non-reentrant path recomputes via saved-tensor hooks
            # instead, so each gradient is reduced exactly once.
...
class SpatialPriorModule(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.with_cp and x.requires_grad:
            # Same DDP constraint as Extractor.forward above (the adapter currently builds this
            # module with with_cp=False, so this path is dormant, but keep the two consistent).
...
class DINOv3_Adapter(nn.Module):
    def __init__(
        ...
        elif getattr(self.backbone, "local_cls_norm", None) is not None:
            # local_cls_norm exists only to satisfy the sat493m checkpoint: DINOv3 applies it to
            # local crops while pretraining, and get_intermediate_layers never touches it. Left
            # trainable it would be an unused parameter, which breaks DDP (find_unused_parameters
            # defaults to False). Freezing it is numerically a no-op here.
            self.backbone.local_cls_norm.requires_grad_(False)
    ...
    def train(self, mode=True):
        # The ViT's only train-mode behaviour is DINOv3's pretraining-time RoPE coordinate
        # augmentation (pos_embed_rope_rescale_coords), which re-scales the position grid by a
        # random factor in [1/2, 2] on every forward. That is an augmentation for pretraining,
        # not for downstream use: it would make a "frozen" extractor non-deterministic and would
        # differ between the frozen and fine-tuned arms. Pin the backbone to eval mode so the two
        # arms differ only in whether its weights receive updates.
        super().train(mode)
        self.backbone.eval()
        return self

    def pretrained_parameters(self):
        # The pretrained ViT only. self.backbone here is deliberately NOT the whole adapter: the
        # SPM, interaction blocks and norms are randomly initialized and must keep the base LR.
    ...
    def forward(self, x):
        ...
        # nullcontext (rather than enable_grad) when fine-tuning: the ambient mode still wins, so
        # inference under torch.no_grad() stays grad-free.
        backbone_grad_context = torch.no_grad() if self.freeze_backbone else contextlib.nullcontext()
        with torch.autocast("cuda", torch.bfloat16):
            with backbone_grad_context:
